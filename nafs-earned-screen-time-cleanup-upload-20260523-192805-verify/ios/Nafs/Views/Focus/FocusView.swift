...
struct FocusView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    if !viewModel.isPremium {
                        ...
                        lockHeroCard
                        earnedTimeStatusCard
                        statusCard
                        toggleCard
                        nextUnlockRequirementCard
                        appSelectionCard
                        lockInControls
                    }
            ...
            .navigationTitle(L10n.text("Nafs Lock", "التركيز"))
            ...
            .alert(L10n.text("Reset Nafs Lock", "إعادة تعيين التركيز"), isPresented: $showResetConfirm) {
            ...
            .sheet(isPresented: $showPremiumGate) {
                UpgradePaywallSheet(
                    ...
                    feature: L10n.text("Nafs Lock", "التركيز"),
                    benefit: L10n.text("Lock distractions and earn freedom through discipline.", "اقفل التطبيقات المشتتة في أوقات الصلاة حتى تصلي."),
    ...
    private var premiumPreview: some View {
        VStack(spacing: 20) {
            VStack(spacing: 10) {
                ...
                Text(L10n.text("Nafs Lock is part of Nafs Premium", "التركيز ضمن نفس بريميوم"))
                    .font(.system(.title2, weight: .bold))
                    .foregroundStyle(NafsTheme.text)
                    .multilineTextAlignment(.center)

                Text(L10n.text("Earn freedom through discipline. Complete worship actions to unlock screen time.", "اقفل التطبيقات المشتتة في أوقات الصلاة حتى تصلي."))
                    .font(.system(.subheadline))
                    .foregroundStyle(NafsTheme.subtleText)
                    .multilineTextAlignment(.center)
            }
            ...
            VStack(spacing: 10) {
                previewRow(icon: "moon.stars.fill", title: L10n.text("Complete Salah to unlock", "يفتح بعد الصلاة"), subtitle: L10n.text("Prayer completion earns screen time and removes prayer lock.", "أكمل الصلاة لتفتح تطبيقاتك."))
                previewRow(icon: "timer", title: L10n.text("Spend earned minutes", "افتح التركيز"), subtitle: L10n.text("Use worship-earned time for intentional unlocks.", "افتح التركيز"))
                previewRow(icon: "app.badge.checkmark", title: L10n.text("Pick your distractions", "اختر مشتتاتك"), subtitle: L10n.text("Choose exactly which apps and categories to lock.", "اختر التطبيقات والفئات التي ستُحجب."))
            }

            NafsButton(title: L10n.text("Unlock Nafs Lock", "افتح التركيز")) {
    ...
    private var entryState: some View {
        VStack(spacing: 20) {
            VStack(spacing: 8) {
                ...
                Text(L10n.text("Choose what Nafs Lock controls", "اقفل التطبيقات أثناء الصلاة"))
                    .font(.system(.title2, weight: .bold))
                    .foregroundStyle(NafsTheme.text)
                    .multilineTextAlignment(.center)

                Text(L10n.text("Your apps unlock when you earn screen time through worship and discipline.", "اختر التطبيقات التي تضيّع فيها وقتك. سيقفلها نفس عند كل صلاة."))
                    .font(.system(.subheadline))
                    .foregroundStyle(NafsTheme.subtleText)
                    .multilineTextAlignment(.center)
            }
    ...
    // MARK: - Lock brand
    private var lockHeroCard: some View {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 5) {
                    Text("Nafs Lock")
                        .font(.system(.title2, weight: .bold))
                        .foregroundStyle(NafsTheme.text)
                    Text("Earn freedom through discipline.")
                        .font(.system(.subheadline, weight: .semibold))
                        .foregroundStyle(NafsTheme.gold)
                }
                Spacer()
                Image(systemName: screenTimeService.activePrayerLock == nil ? "lock.shield.fill" : "lock.fill")
                    .font(.system(size: 34, weight: .semibold))
                    .foregroundStyle(NafsTheme.gold)
            }

            Text("Complete worship actions to unlock screen time. Available minutes are earned, not given.")
                .font(.system(.caption))
                .foregroundStyle(NafsTheme.subtleText)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var earnedTimeStatusCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Available Earned Screen Time")
                ...
                .disabled(viewModel.discipline.credits.currentAvailableMinutes < 15)
                .opacity(viewModel.discipline.credits.currentAvailableMinutes < 15 ? 0.45 : 1)
            }

            Text(lockMessage ?? "Complete Salah, Quran, Dhikr, or Nafs Lock to earn more.")
    ...
    private var nextUnlockRequirementCard: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text("Next Unlock Requirement")
                    ...
                    .foregroundStyle(NafsTheme.text)
                Text(nextUnlockRequirementText)
                    .font(.system(.caption))
                    .foregroundStyle(NafsTheme.subtleText)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var appSelectionCard: some View {
        VStack(spacing: 14) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(L10n.text("Apps Currently Locked", "التطبيقات المحجوبة"))
                        .font(.system(.headline, weight: .bold))
                        .foregroundStyle(NafsTheme.text)
    ...
    private var lockInControls: some View {
        VStack(spacing: 10) {
            Button {
                screenTimeService.applyDisciplineShields()
                viewModel.recordFocusCompleted(minutes: 15)
                lockMessage = "Locked in. Discipline earns freedom."
            } label: {
                HStack(spacing: 8) {
    ...
    private var hardResetButton: some View {
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: "arrow.counterclockwise")
                    .font(.system(.caption, weight: .semibold))
                Text(L10n.text("Hard Reset Blocks", "إعادة تعيين جميع الحجب"))
    ...
    private var statusText: String {
        if let activePrayer = screenTimeService.activePrayerLock {
            return L10n.text("Locked until you complete \(NafsStrings.prayerName(activePrayer)) or spend earned time.", "مقفل من أجل \(NafsStrings.prayerName(activePrayer)). أكمل صلاتك للفتح.")
        }
        return L10n.text("Ready. Apps lock at prayer time or when you Lock In.", "تُقفل التطبيقات تلقائياً عند كل صلاة.")
    }

    private var nextUnlockRequirementText: String {
        if let activePrayer = screenTimeService.activePrayerLock {
            return "Complete \(NafsStrings.prayerName(activePrayer)) to unlock, or spend earned screen time."
        }
        if viewModel.discipline.credits.currentAvailableMinutes >= 15 {
            return "Spend 15 earned minutes for an intentional unlock."
        }
        return "Complete Salah to unlock. Quran and Dhikr also earn minutes."
    }

    private func spendEarnedUnlock(minutes: Int) {
        guard viewModel.discipline.spendCredits(minutes: minutes) else {
            lockMessage = "Complete worship actions to earn unlock minutes."
            return
        }
        screenTimeService.temporaryUnlock(minutes: minutes)
        unlockSuccess.toggle()
        lockMessage = "Unlocked for \(minutes) min. Apps will re-lock automatically."
    }
